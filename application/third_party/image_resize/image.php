...
// crop (optional) force crop to get the ratio width/height //old cropratio	(optional) ratio of width to height to crop final image (e.g. 1:1 or 3:2)
...
define('CACHE_DIR_NAME',		'/../../../uploads/images/imagecache/');
...
$image_pre = explode('/uploads',$_SERVER['REQUEST_URI']);
...
if(strstr($_SERVER["SERVER_NAME"],"nitsnets.com")||(strstr($_SERVER["SERVER_NAME"],"servidorbeta.com")))
{ 
$docRoot = str_replace('.servidorbeta.com','',$_SERVER["SERVER_NAME"]);
$docRoot = str_replace('.nitsnets.com','',$docRoot);

$subdomain = explode('.',$_SERVER["SERVER_NAME"]);
 
if (count($subdomain) >= 4) 
{ 
	$subdomain = $subdomain[0];
	$docRoot = str_replace($subdomain.'.','',$docRoot);
}	else
{
		$subdomain = "demo";
}

$docRoot = DOCUMENT_ROOT.'webs/'.$docRoot.'/'.$subdomain.'/';
...
{
    $image = preg_replace("/media\/(.*)jpg/", "media/default.jpg", $image);
    $image = preg_replace("/games\/(.*)jpg/", "games/default.jpg", $image);
    $image = preg_replace("/banners\/(.*)jpg/", "banners/default.jpg", $image);
...
//if ((!$maxWidth && !$maxHeight) || (!$color && FALSE))
...
if (isset($_GET['crop']))
{ 
	// old line: $cropRatio		= explode(':', (string) $_GET['cropratio']);
        $cropRatio = array($_GET['width'],$_GET['height']);     
...
if (isset($_GET['fill']))
{
        ...
        $fillRatio = array($_GET['width'],$_GET['height']);

	if (count($fillRatio) == 2)
	{
		$ratioComputed		= $width / $height;
		$fillRatioComputed	= (float) $fillRatio[0] / (float) $fillRatio[1];

		if ($ratioComputed < $fillRatioComputed)
		{ // Image is too tall so we will crop the top and bottom

			$origHeight	= $height;
			$heightFill		= $width / $fillRatioComputed;
			$offsetY	= (($origHeight - $heightFill) *  $fillRatioComputed) / 2;
   
            $height = $origHeight;
                        
		} 
		else if ($ratioComputed > $fillRatioComputed)
		{ // Image is too wide so we will crop off the left and right sides
    
			$origWidth	= $width;
			$widthFill		= $height * $fillRatioComputed;
			$offsetX	= (($origWidth - $widthFill)  /  $fillRatioComputed) / 2;
                        $width = $origWidth;
...
//Get fills squeares 
$addHeightFill = $offsetX*$xRatio;
$addWidthFill = $offsetY*$yRatio;
...
/*echo "X:".$offsetX." ratio (".$xRatio.") Y:".$offsetY." ratio (".$yRatio.") <br>";
echo "TamFinal X = ".$tnWidth." and TamFinal Y = ".$tnHeight."<br>";
echo "Aumentar Y=".($offsetX*$xRatio)." Aumentar X= ".($offsetY*$yRatio);die();*/
...
if (isset($_GET['crop']))
	$resizedImageSource	.= 'x' . (string) $_GET['crop'];
    
if (isset($_GET['instagraph']))
	$resizedImageSource	.= 'xi' . (string) $_GET['instagraph'];

if (isset($_GET['fill']))
	$resizedImageSource	.= 'x' . (string) $_GET['fill'];
...
// Fill the image if is necessary
if (isset($_GET['fill']))
{
 
// Set up a blank canvas for our resized image (destination)
$dst  = imagecreatetruecolor($tnWidth+($addWidthFill*2), $tnHeight+($addHeightFill*2));


// Set background color
if(isset($_GET['fillcolor']))
    $color = $_GET['fillcolor'];
    else
            $color = "FFFFFF";

$color = imagecolorallocate($dst, hexdec($color[0].$color[1]), hexdec($color[2].$color[3]), hexdec($color[4].$color[5]));
imagefilledrectangle ($dst, 0, 0, $tnWidth+($addWidthFill*2), $tnHeight+($addHeightFill*2),$color);

// Paste original image
imagecopyresampled ($dst, $src, $addWidthFill, $addHeightFill, 0, 0, $tnWidth, $tnHeight, $width, $height);
...
try 
{
    $instagraph = Instagraph::factory($resized,$resized);
}
catch (Exception $e) 
{
    echo $e->getMessage();
